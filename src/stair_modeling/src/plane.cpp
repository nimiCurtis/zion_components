...
// Compute the plane's principal directions using covariance matrix
void Plane::getPrincipalDirections() 
{
    ...
    plane_dir_.col(2) = plane_dir_.col(0).cross(plane_dir_.col(1));  // Column 0 has smaller eigenvalues, thus direction is perpendicular to the plane
}
...
void Plane::getMeasurements() 
{
    ...
    Eigen::Matrix4f cp2p(Eigen::Matrix4f::Identity()); // transformation: cp2p = camera projected ==> plane
